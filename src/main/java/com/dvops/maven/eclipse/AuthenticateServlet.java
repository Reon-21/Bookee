...
public class AuthenticateServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		for (int i = 0; i < registeredUserList.size(); i++) {
			if (registeredUserList.get(i).getEmail().toString().equals(email)
					&& registeredUserList.get(i).getPassword().toString().equals(password)) {
				login = true;
		...
		if (login) {
			...
			writer.println("window.location.href = 'http://localhost:8091/DevOps-Project/hotels.jsp';");
			writer.println("</script>");
		} else {
			writer.println("<script type=\"text/javascript\">");
			writer.println("alert('Incorrect email or password');");
			writer.println("window.location.href = 'http://localhost:8091/DevOps-Project/login.jsp';");
			writer.println("</script>");
		}
